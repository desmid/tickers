    ...
    def name2posn(self, n=''):
        ...
        while len(n) > 0:
            ...
            if v < self.ORD_A:
                r = int(n)
                break
            c = 26*c + v-self.ORD_A+1
            n = n[1:]
        if c > 0: c -= 1
        if r > 0: r -= 1
        return (c, r)

    def range2posn(self, n=''):
        ...
        """
        if isinstance(n, int) or not isinstance(n, str):
            n = self.name2posn(n)
            return (n, n)
        try:
            c,r = n.split(':', 1)
        ...
        if c == '': c = '0'
        if r == '': r = '0'
        return (self.name2posn(c), self.name2posn(r))


    def posn2name(self, p):
        ...
        while col > 0:
            ...
            name = chr(self.ORD_A + remainder) + name
